...
namespace margelo::nitro::nitrotflite {
  ...
  public:
    explicit HybridTfliteModel(TfLiteInterpreter* interpreter,
    ...
    std::vector<std::shared_ptr<ArrayBuffer>> runSync(
        const std::vector<std::shared_ptr<ArrayBuffer>>& input) override;
    std::shared_ptr<Promise<std::vector<std::shared_ptr<ArrayBuffer>>>> run(
        const std::vector<std::shared_ptr<ArrayBuffer>>& input) override;
